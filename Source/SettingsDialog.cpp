...
INT_PTR SettingsDialog::OnInitDialog() {
	...
	if (mPluginSettings->asTag == 1) {
		Button_SetCheck(::GetDlgItem(m_hwnd, IDC_CHECK_ASTAG),TRUE);
	}

	if (mPluginSettings->wholeWord == 1) {
		Button_SetCheck(::GetDlgItem(m_hwnd, IDC_CHECK_WHOLEWORD),TRUE);
	}
...
void SettingsDialog::SaveAllSettings() {
	...
	mPluginSettings->autoShowCapture = (checked == BST_CHECKED) ? 1 : 0;

	checked = Button_GetCheck(::GetDlgItem(m_hwnd, IDC_CHECK_ASTAG));
	mPluginSettings->asTag = (checked == BST_CHECKED) ? 1 : 0;

	checked = Button_GetCheck(::GetDlgItem(m_hwnd, IDC_CHECK_WHOLEWORD));
	mPluginSettings->wholeWord = (checked == BST_CHECKED) ? 1 : 0;
