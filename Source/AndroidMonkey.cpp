...
bool AndroidMonkey::onStart() {
    Sleep(1000);
    //sendMonkeyCmd("press HOME");
    return true;
}

void AndroidMonkey::onExit() {
    quitMonkey();
    release();
}

bool AndroidMonkey::onLoop() {
    int size = mMonkeyCmds.size();

    for (int i=0; i < size; i++) {
        std::string cmd = mMonkeyCmds[i];
        if (getMonkeyCmd(cmd) != -1) {
            sendMonkeyCmd(cmd.c_str());
        }
    }
